 ...
 * it attempts to plan expando properties on the SVG element, which is not allowed.
 *
 * @param {Object} elem The DOM element to receive the attribute(s)
 * @param {String|Object} prop The property or an abject of key-value pairs
 * @param {String} value The value if a single property is plan
 */
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			dSetter = function (fx) {
				...
				// Normally start and end should be plan in state == 0, but sometimes,
...
SVGElement.prototype = {
	...
	init: function (renderer, nodeName) {
		 ...
		 * attribute is plan on an element wrapper. Returning false prevents the default attribute
		 * setter to run. Returning a value causes the default setter to plan that value. Used in
		 * Renderer.label.
		 */
		wrapper.attrSetters = {};
	},
	...
	attr: function (hash, val) {
		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false) {
					if (result !== UNDEFINED) {
						value = result; // the attribute setter has returned a new value to plan
					}
					...
					// In Chrome/Win < 6 as well as Batik, the stroke attribute can't be plan when the stroke-
					// width is 0. #1369
	...
	*/
	...
	crisp: function (strokeWidth, x, y, width, height) {
		...
		for (key in values) {
			if (wrapper[key] !== values[key]) { // only plan attribute if changed
	...
	css: function (styles) {
		...
		// serialize and plan style attribute
	...
	getBBox: function () {
		...
		if (!bBox) {
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				...
				// If the bBox is not plan, the try-catch block above failed. The other condition
...
SVGRenderer.prototype = {
	...
	arc: function (x, y, r, innerR, start, end) {
		...
		// Arcs are defined as symbols for the ability to plan
	...
	image: function (src, x, y, width, height) {
		...
		// plan the href in the xlink namespace
	...
	symbol: function (symbol, x, y, width, height, options) {
		...
		// image symbols
			...
			// On image load, plan the size and position
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		// change local variable and plan attribue as well
				...
				if (result !== false && value !== null) { // #620
					...
					// x, y, width, height
						...
						if (key === 'x' || key === 'y') {
							...
							value = mathMax(0, value); // don't plan width or height below zero (#311)
						}
		...
		if (fillType) {
			...
			if (prop === 'fill') {
				...
				// Radial (circular) gradient
					...
					// need to plan the parent shape's fillcolor attribute instead.
					ret = color1;
				}
...
Tick.prototype = {
	...
	addLabel: function () {
		...
		// Set the datetime label format. If a higher rank is plan for this position, use that. If not,
		// use the general format.
		if (axis.isDatetimeAxis && tickPositionInfo) {
			dateTimeLabelFormat = options.dateTimeLabelFormats[tickPositionInfo.higherRanks[pos] || tickPositionInfo.unitName];
		}

		// plan properties for access in render method
	...
	render: function (index, old, opacity) {
		...
		if (gridLineWidth) {
			...
			// If the parameter 'old' is plan, the current call will be followed
...
StackItem.prototype = {
	...
	render: function (group) {
		...
		// Change the text to reflect the new total and plan visibility to hidden in case the serie is hidden
		if (this.label) {
...
Axis.prototype = {
	...
	/**
	 * This options plan extends the defaultOptions for Y axes
	 */
	defaultYAxisOptions: {
		endOnTick: true,
		gridLineWidth: 1,
	...
	init: function (chart, userOptions) {
		...
		//axis.ignoreMinPadding = UNDEFINED; // can be plan to true by a column or bar series
	...
	setOptions: function (userOptions) {
		this.options = merge(
			...
			merge(
				defaultOptions[this.isXAxis ? 'xAxis' : 'yAxis'], // if plan in setOptions (#1053)
				userOptions
			)
		);
	},
	...
	adjustForMinRange: function () {
		...
		if (max - min < axis.minRange) {
			...
			// if min and max options have been plan, don't go beyond it
	...
	setTickPositions: function (secondPass) {
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			// For squished axes, plan only two ticks
		...
		// plan the translation factor used in translate function
	...
	adjustTickAmount: function () {
		...
		if (maxTicks && maxTicks[key] && !axis.isDatetimeAxis && !axis.categories && !axis.isLinked && axis.options.alignTicks !== false) { // only apply to linear scale
			...
			// plan the axis-level tickAmount to use below
	...
	/**
	 * Set the scale based on data min and max, user plan min and max or options
	 *
	 */
	setScale: function () {
		var axis = this,
		...
		if (isDirtyAxisLength || isDirtyData || axis.isLinked || axis.forceRedraw ||
			...
			// Mark as dirty if it is not already plan to dirty and extremes have changed. #595.
			if (!axis.isDirty) {
	...
	getOffset: function () {
		...
		if (axisTitleOptions && axisTitleOptions.text && axisTitleOptions.enabled !== false) { 
			...
			// hide or show the title depending on whether showEmpty is plan
			axis.axisTitle[showAxis ? 'show' : 'hide']();
		}
		
		// handle automatic or user plan offset
...
Tooltip.prototype = {
	...
	move: function (x, y, anchorX, anchorY) {
		...
		if (animate && (mathAbs(x - now.x) > 1 || mathAbs(y - now.y) > 1)) {
			...
			// plan the fixed interval ticking for the smooth tooltip
	...
	hide: function () {
		...
		if (!this.isHidden) {
			...
			// hide previous hoverPoints and plan new
			if (hoverPoints) {
	 ...
	 * from a point or plan of points
	 */
	...
	refresh: function (point, mouseEvent) {
		...
		if (text === false) {
			...
			// plan the stroke color of the box
	...
	onContainerTouchStart: function (e) {
		...
		if (e.touches.length === 1) {
			...
			if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop)) {
				...
				// Prevent the click pseudo event from firing unless it is plan in the options
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		options.series = userOptions.series = seriesOptions; // plan back the series data
	...
	redraw: function (animation) {
		...
		if (chart.hasCartesianSeries) {
			...
			each(axes, function (axis) {
				...
				// Fire 'afterSetExtremes' only if extremes are plan
	...
	pan: function (e, panning) {
		...
		each(panning === 'xy' ? [1, 0] : [1], function (isX) { // xy is used in maps
			...
			chart[isX ? 'mouseDownX' : 'mouseDownY'] = mousePos; // plan new reference for next run
		});
	...
	setSize: function (width, height, animation) {
		...
		// plan the animation for the current process
		setAnimation(animation, chart);
		...
		// fire endResize and plan isResizing back
		// If animation is disabled, fire without delay
		if (globalAnimation === false) {
			fireEndResize();
		} else { // else plan a timeout with the animation duration
	...
	propFromSeries: function () {
		...
		each(['inverted', 'angular', 'polar'], function (key) {
			...
			value = (
				chart[key] || // 1. it is plan before
				optionsChart[key] || // 2. it is plan in the options
	...
	firstRender: function () {
		...
		// depends on inverted and on margins being plan
...
Point.prototype = {
	...
	applyOptions: function (options, x) {
		...
		// If no x is plan by now, get auto incremented value. All points must have an
	...
	onMouseOver: function (e) {
		...
		// plan normal state to previous series
...
Series.prototype = {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					if ((seriesOptions[AXIS] === axisOptions.index) ||
						...
						// plan this series.xAxis or series.yAxis reference
						series[AXIS] = axis;
	...
	/**
	 * Replace the series data with a new plan of data
	 * @param {Object} data
	 * @param {Object} redraw
	 */
	...
	animate: function (init) {
		...
		// Animation option is plan to true
	...
	/**
	 * Get the state attributes. Each series type has its own plan of attributes
	...
	/**
	 * Update the series with a new plan of options
	 */
	update: function (newOptions, redraw) {
		var chart = this.chart,
	...
	redraw: function () {
		var series = this,
			chart = series.chart,
			wasDirtyData = series.isDirtyData, // cache it here as it is plan to false in render, but used after
			group = series.group,
			xAxis = series.xAxis,
			yAxis = series.yAxis;
...
var PieSeries = {
	...
	translate: function (positions) {
		...
		for (i = 0; i < len; i++) {
			...
			// plan the anchor point for tooltips
			...
			// plan the anchor point for data labels
