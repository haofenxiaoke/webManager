...
for(var i=0;i<$("#love-course-list li").length;i++){
    ...
    if(y!=yss){
        ...
        if(y>yss){
            ...
            var t = '<a class="appointment DBD"><div class="appointment-ico"></div><span>预约直播</span><div class="prompt">课前<div>5分钟</div>会短信提醒你上课</div></a>'
            $(".addbf").eq(i).html(t)
        }
    }else if(y==yss){
        ...
        if(m<mss){
            ...
            if(d<dss){
                ...
                if((parseInt(hour)*60+parseInt(min))>=(parseInt(hour1)*60+parseInt(min1))&&(parseInt(hour)*60+parseInt(min))<=(parseInt(hour2)*60+parseInt(min2))) {
                    var t = '<div class="streaming" style="overflow: hidden;"><div class="sku-play"><span class="sku-one"></span> <span class="sku-two"></span> <span class="sku-three"></span></div><span>正在直播</span></div>'
                    $(".addbf").eq(i).html(t)
    ...
    if(y==yss&&m==mss&&d==dss){
        $("#love-course-list li:eq("+i+") .mart").html("今天"+"  "+hour1+":"+min1+""+"-"+hour2+":"+min2)
        if((parseInt(hour)*60+parseInt(min))>=(parseInt(hour1)*60+parseInt(min1))&&(parseInt(hour)*60+parseInt(min))<=(parseInt(hour2)*60+parseInt(min2))) {
            var t = '<div class="streaming" style="overflow: hidden;"><div class="sku-play"><span class="sku-one"></span> <span class="sku-two"></span> <span class="sku-three"></span></div><span class="now">正在直播</span></div>'
            $(".addbf").eq(i).html(t)
        }else if((parseInt(hour)*60+parseInt(min))<(parseInt(hour1)*60+parseInt(min1))){
            var t = '<a class="appointment DBD"><div class="appointment-ico"></div><span>预约直播</span><div class="prompt">课前<div>5分钟</div>会短信提醒你上课</div></a>'
            $(".addbf").eq(i).html(t)
