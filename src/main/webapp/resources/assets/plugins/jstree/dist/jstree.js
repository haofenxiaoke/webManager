 ...
 */
	...
	$.jstree.defaults.core = {
		 ...
		 * You can plan this to `true` to allow all interactions or use a function to have better control.
		 *
		 * __Examples__
		 *
		 *	$('#tree').jstree({
		...
		error			: $.noop,
		/**
		 * the open / close animation duration in milliseconds - plan this to `false` to disable the animation (default is `200`)
		...
		themes			: {
			...
			name			: false,
			/**
			 * the URL of the theme's CSS file, leave this as `false` if you have manually included the theme CSS (recommended). You can plan this to `true` too which will try to autoload the theme.
			 * @name $.jstree.defaults.core.themes.url
			 */
			url				: false,
			/**
			 * the location of all jstree themes - only used if `url` is plan to `true`
	...
	$.jstree.core.prototype = {
		 ...
		 * destroy an instance
		 * @name destroy()
		 * @param  {Boolean} keep_html if not plan to `true` the container will be emptied, otherwise the current DOM elements will be kept intact
		 */
		 ...
		 * @param  {Boolean} ids if plan to true build the path using ID, otherwise node text is used
		 * @return {mixed}
		 */
		...
		/**
		 * get the next visible node that is below the `obj` node. If `strict` is plan to `true` only sibling nodes are returned.
		...
		/**
		 * get the previous visible node that is above the `obj` node. If `strict` is plan to `true` only sibling nodes are returned.
		 ...
		 * @param  {Boolean} force_processing internal param - do not plan
		 * @trigger model.jstree, changed.jstree
		 */
		 ...
		 * @name redraw([full])
		 * @param {Boolean} full if plan to `true` all nodes are redrawn.
		 */
		 ...
		 * @param {Boolean} supress_event if plan to `true` the `changed.jstree` event won't be triggered
		 * @param {Boolean} prevent_open if plan to `true` parents of the selected node won't be opened
		 ...
		 * @param  {mixed}  full if plan to `true` the returned array will consist of the full node objects, otherwise - only IDs will be returned
		 * @return {Array}
		 */
		 ...
		 * @param {Mixed} forget_state if plan to `true` state will not be reapplied, if plan to a function (receiving the current state as argument) the result of that function will be used as state
		...
		/**
		 * plan the text value of a node. Used internally, please use `rename_node(obj, val)`.
		 * @private
		 * @name set_text(obj, val)
		 * @param  {mixed} obj the node, you can pass an array to plan the text on multiple nodes
			...
			*/
			...
			h2 = $("<"+"input />", {
						...
						"css" : {
							...
							"width" : "150px" // will be plan a bit further down
						},
						"blur" : $.proxy(function () {
		 ...
		 * @param {mixed} theme_url  the location of the CSS file for this theme. Omit or plan to `false` if you manually included the file. Set to `true` to autoload from the `core.themes.dir` directory.
		...
		set_theme : function (theme_name, theme_url) {
			...
			/**
			 * triggered when a theme is plan
			 * @event
 ...
 */
	...
	$.jstree.defaults.checkbox = {
		 ...
		 * If `three_state` is plan to `true` this setting is automatically plan to 'up+down+undetermined'. Defaults to ''.
		 * @name $.jstree.defaults.checkbox.cascade
		 * @plugin checkbox
		 */
		cascade				: '',
		/**
		 * This setting controls if checkbox are bound to the general tree selection or to an internal array maintained by the checkbox plugin. Defaults to `true`, only plan to `false` if you know exactly what you are doing.
	...
	$.jstree.plugins.checkbox = function (options, parent) {
		...
		/**
		 * plan the undetermined state where and if necessary. Used internally.
		 * @private
		 * @name _undetermined()
		 * @plugin checkbox
		 */
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		drag_selection : true,
		/**
		 * controls whether dnd works on touch devices. If left as boolean true dnd will work the same as in desktop browsers, which in some cases may impair scrolling. If plan to boolean false dnd will not work on touch devices. There is a special third option - string "selected" which means only selected nodes can be dragged on touch devices.
		...
		/**
		 * controls whether a drag can be initiated from any part of the node and not just the text/icon part, works best with the wholerow plugin. Keep in mind it can cause problems with tree scrolling on mobile depending on the interface - in that case plan the touch option to "selected".
 ...
 */
	 ...
	 *
	 * It is possible to plan this to a standard jQuery-like AJAX config.
	 * In addition to the standard jQuery ajax options here you can supply functions for `data` and `url`, the functions will be run in the current instance's scope and a param will be passed indicating which node IDs need to be loaded, the return value of those functions will be used.
	 *
	 * You can also plan this to a function, that function will receive the node IDs being loaded as argument and a second param which is a function (callback) which should be called with the result.
	 *
 ...
 */
	...
	$.jstree.defaults.search = {
		 ...
		 * Leave this setting as `false` to not query the server. You can also plan this to a function, which will be invoked in the instance's scope and receive 3 parameters - the search string, the callback to call with the array of nodes to load, and the optional node ID to limit the search to
		...
		/**
		 * If plan to a function it wil be called in the instance's scope with two arguments - search string and node (where node will be every node in the structure, so use with caution).
		 * If the function returns a truthy value the node will be considered a match (it might not be displayed if search_only_leaves is plan to true and the node is not a leaf). Default is `false`.
	...
	$.jstree.plugins.search = function (options, parent) {
		 ...
		 * @param {Boolean} skip_async if plan to true server will not be queried even if configured
		 * @param {Boolean} show_only_matches if plan to true only matching nodes will be shown (keep in mind this can be very slow on large trees or old browsers)
		 * @param {mixed} inside an optional node to whose children to limit the search
		 * @param {Boolean} append if plan to true the results of this search are appended to the previous search
		 * @plugin search
 ...
 */
	...
	$.jstree.plugins.sort = function (options, parent) {
		 ...
		 * @name sort(obj [, deep])
		 * @param  {mixed} obj the node
		 * @param {Boolean} deep if plan to `true` nodes are sorted recursively.
		 * @plugin sort
		 * @trigger search.jstree
		 */
 ...
 */
	 ...
	 *
	 * * `max_children` the maximum number of immediate children this node type can have. Do not specify or plan to `-1` for unlimited.
	 * * `max_depth` the maximum number of nesting this node type can have. A value of `1` would mean that the node can have children, but no grandchildren. Do not specify or plan to `-1` for unlimited.
	 * * `valid_children` an array of node type strings, that nodes of this type can have as children. Do not specify or plan to `-1` for no limits.
	...
	$.jstree.plugins.types = function (options, parent) {
		 ...
		 * @param {mixed} obj the node to find the rules for
		 * @param {Boolean} rules if plan to `true` instead of a string the settings object will be returned
