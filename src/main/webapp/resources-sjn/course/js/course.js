...
(function ($,window) {
    ...
    function bindEvents() {
        ...
        $('.kjsmore').click(function () {
            if($(this).attr('more')=='true'){
                ...
                if(dataType == 2){
                    $('#industry-more').css("display","none")
                    $("#i-more").text("展开")
                    //$('#industry-more').html('');
                }
                if(dataType == 3){ //$('#position-more').html('');
                    $('#position-more').css("display","none")
                    $("#p-more").text("展开")
                }
                $(this).css("color", "#999");
                ...
                if(!flag){
                    ...
                    $(this).parent().find("a").eq(0).css('color','#333333');
    ...
    function bindEvents2() {
        ...
        $('#direction a').unbind().click(function () {
            ...
            _direction.find("a").css("border", "");
            $(this).css("border", "1px solid #cccccc").siblings().css("border", "");

            _industry.find("a").eq(0).css("border", "1px solid #cccccc");
            _position.find("a").eq(0).css("border", "1px solid #cccccc");
       ...
       /* $('#industry a').unbind().click(function () {
            $('#industry').find("a").css("border", "")
            $(this).css("border", "1px solid #cccccc").siblings().css("border", "");

            $('#industry').find("a").attr('select', 'false');
            $(this).attr('select', 'true');

            getCourses();
        });

        $('#position a').unbind().click(function () {
            $('#position').find("a").css("border", "")
            $(this).css("border", "1px solid #cccccc").siblings().css("border", "");

            $('#position').find("a").attr('select', 'false');
            $(this).attr('select', 'true');
        ...
        $('.kjsCrnum1 a').each(function(){
            ...
            if($(this).attr('select')=='true'){
                ...
                if(dataValue){
                    ...
                    if(dataType == 1 && dataValue) datas['directionId']=dataValue;
                    if(dataType == 2 && dataValue) datas['industryDesc']=dataValue;
                    if(dataType == 3 && dataValue) datas['positionId']=dataValue;
                }
            }
        })

        //排序类型 sortType
        $('.kjsPx a').each(function(){
            if($(this).attr('select')=='true'){
                datas['sortName']=$(this).attr('dataType');
            }
        })
        return datas;
    }
                ...
                if (result.success) {
                    ...
                    if(!result.data){
                        ...
                        $.each(result.data, function(index, course) {
                            var vipStr = nullVal,percent=1,isVIP = $('#is-vip').val();
                            if(isVIP == 'false'){
                                vipStr =
                                ...
                                vipStr =
                                    ...
                                    '</div>'+
                                    '<div class="kjsSmallj">学习了<span style="color: #e62c3a;">{{finishLectureNum}}</span>/{{lectureNum}}课程小节</div>';
                            }
                            ...
                            $.each(new Array(5),function (index) {
                                if(parseInt(course.score)>4){
                                    kjsPf +='<a class="sjn_restBG kjsXxcur"></a>';
                                }else{
    ...
    function fillMore(dataType){
        ...
        $.get(url).done(function (result) {
            if (result.success) {
                ...
                if(dataType == 3){ $('#position-more').html(htmlContent);
                    ...
                    $.post('/course/list', datas).done(function (result) {
                        ...
                        if (result.success) {
                            ...
                            if(!result.data){
                                ...
                                htmlContent = '<ul class="clearfloat">'+htmlContent+'</ul>';
                                $('.kjsCrWid').append('<div class="kjsCrouse bb" style="-webkit-transition: all .5s">'+htmlContent+'</div>');
                                /*$('.kjsCrouse').last().hide();*/
