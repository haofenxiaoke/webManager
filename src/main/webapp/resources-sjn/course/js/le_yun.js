...
function initData(){
    //延迟0.2s后播放
    setTimeout(function(){
        if(isvipVideo == 0 && isVIP == 'false') return;
        if(playType==0){
            play(vid,'playerswf');
        }else if(playType==1){
    ...
    try{
        ...
        $.post('/course/save/video', datas).done(function (result) {
            if (result.success) {
                ...
                //console.log('视频保存成功'+new Date().getTime())
            }else{
            ...
            if(nowLectureId == lectureId){
                if( playType==0){
                    ...
                    if(player.getPlayState().finish){
                        ...
                        if((index+1 == $('#video-id li').size()) || (_isvipVideo == 0 && isVIP == false) ){
...
$('.onesrk textarea').on('click',function(e){
    $(this).parent().css({'border':'1px solid #999999','background':'#fff'});
