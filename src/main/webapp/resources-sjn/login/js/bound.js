...
(function ($) {
    ...
    function onRegisterSuccess() {
        ...
        $.get('/resources-sjn/modal/bound-success.html').done(function (html) {
            var time = 3;
            layer.closeAll();
            layer.open({
                offset: '40%',
    ...
    function bindEvents() {
        ...
        function validateFormBinds() {
            validateBind('phone', function (val, tips) {
                $('#image-captcha').siblings('.positioning').html('');
                if (val == '') {
                    tips.html('请输入手机号');
...
//IE8情况下使用Placeholder插件绑定实际的INPUT
           ...
           /* $(".password-number input").focus(function() {
                var pasd = $(".password-number input").val();

                pasd = "";
                if (pasd == "") {
                    $(".password-number input[type='text']").attr('type', 'password')
                }
            })
            $(".password-number input").blur(function() {
                var pasd = $(".password-number input").val();

                if (pasd == "" || pasd == "密码6~18位") {
                    $(".password-number input").text("密码6~18位");
                    $(".password-number input").attr('type', 'text')
                }
